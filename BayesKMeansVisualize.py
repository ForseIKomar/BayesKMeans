...
class BayesKMeansVisualize(object):
    ...
    def showColoredData(self):
        clusterer = KMeans(n_clusters=self.bayesKMeans.foundK, random_state=self.bayesKMeans.method.random_state)
    ...
    def showBayesianPlot(self, line=True):
        ...
        if line:
            plt.plot(x_asc, y_asc, label=self.bayesKMeans.methodName)

        plt.scatter(x_asc, y_asc, label=self.bayesKMeans.methodName + ' points', marker='x', s=15, c='green')
        plt.scatter(self.bayesKMeans.foundK, d[self.bayesKMeans.foundK], c='red', marker='+',
                    s=35, label='Target = ' + str(self.bayesKMeans.foundK))
